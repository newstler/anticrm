...
<script lang="ts">
  ...
  let prevAssigned: Ref<Collaborator>[] = []
  let componentLead: Ref<Collaborator> | undefined = undefined
  let members: Ref<Collaborator>[] = []
  let docQuery: DocumentQuery<Collaborator> = {}
  ...
  async function updateComponentMembers (project: Project, issue: Object) {
    ...
    if (project !== undefined) {
      ...
      members = accounts.map((p) => p.person)
    } else {
      members = []
    }
  ...
  const handleAssigneeChanged = async (newAssignee: Ref<Collaborator> | undefined) => {
