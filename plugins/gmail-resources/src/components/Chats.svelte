...
<script lang="ts">
  ...
  async function share (): Promise<void> {
    ...
    await client.addCollection(
      ...
      'gmailSharedMessages',
      {
        messages: convertMessages(object, channel, selectedMessages, $personAccountByIdStore, $collaboratorByIdStore)
      }
    )
...
<Scroller>
  <div class="popupPanel-body__main-content py-4 clear-mins flex-no-shrink">
    {#if messages && messages.length > 0}
      <Messages
        messages={convertMessages(object, channel, messages, $personAccountByIdStore, $collaboratorByIdStore)}
        {selectable}
        bind:selected
        on:select
      />
