<script lang="ts">
  ...
  import { PersonAccount, getName as getContactName } from '@hcengineering/contact'
  ...
  function getName (a: Attachment, personAccountByIdStore: IdMap<PersonAccount>): string | undefined {
    const acc = personAccountByIdStore.get(a.modifiedBy as Ref<PersonAccount>)
    if (acc !== undefined) {
      const emp = $collaboratorByIdStore.get(acc?.person)
      if (emp !== undefined) {
      ...
      <div class="attachmentContainer flex-no-shrink clear-mins" on:click={() => openAttachment(att)}>
        ...
        <div class="label">
          <Label
            ...
            params={{ name: getName(att, $personAccountByIdStore), time: getTime(att.modifiedOn) }}
          />
        </div>
      </div>
    {/each}
