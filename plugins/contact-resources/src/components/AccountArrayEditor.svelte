...
-->
<script lang="ts">
  import contact, { Collaborator, PersonAccount } from '@hcengineering/contact'
  ...
  ).filter((it) => it !== undefined) as Ref<Collaborator>[]

  $: docQuery =
    excluded.length > 0
      ? {
          active: true,
          _id: { $nin: excluded.map((p) => (p as PersonAccount).person as Ref<Collaborator>) }
        }
      : {
          active: true
        }
